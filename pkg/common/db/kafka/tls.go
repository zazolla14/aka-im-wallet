// Copyright © 2024 AkaIM open source community. All rights reserved.
//
...
func decryptPEM(data, passphrase []byte) (decrypted []byte, err error) {
	if len(passphrase) == 0 {
		return data, nil
	}
	b, _ := pem.Decode(data)

	// TODO: should change to pkcs8.ParsePKCS8PrivateKey
	//nolint:staticcheck // SA1019: DecryptPEMBlock is deprecated but still used for backward compatibility with legacy PEM files
...
func newTLSConfig(clientCertFile, clientKeyFile, caCertFile string, keyPwd []byte, insecureSkipVerify bool) (*tls.Config, error) {
	...
	//nolint:gosec // InsecureSkipVerify is enabled only for development with self-signed certificates
